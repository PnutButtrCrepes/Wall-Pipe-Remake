...
public class BlockFieldManager extends Entity {
	...
	public void addBlock(BuildingBlock block, int xIndex, int yIndex) {
		...
		blockField[xIndex - 2][yIndex - 2].x = xIndex;
		blockField[xIndex - 2][yIndex - 2].y = yIndex;
	}
	...
	public void act(float delta) {
		...
		if(replacing) {
			...
			if(blockBreakTimer <= 0) {
				...
				blockField[replacementXIndex - 2][replacementYIndex - 2].x = replacementXIndex;
				blockField[replacementXIndex - 2][replacementYIndex - 2].y = replacementYIndex;
				...
				Gdx.input.setCursorPosition((int) ((gameScene.nbManager.getX() + 24) * gameScene.screenWidthRatio),
						(int) (gameScene.viewport.getScreenHeight() - ((gameScene.nbManager.getY() + 24) * gameScene.screenHeightRatio)));
